...
func (s *stubRuleStore) ListActiveRulesForScope(ctx context.Context, orgID uuid.UUID, namespace, surface, segmentID string, ts time.Time) ([]types.Rule, error) {
	copied := make([]types.Rule, len(s.rules))
	copy(copied, s.rules)
	return copied, nil
}
...
func TestBuildResponseAppliesRuleEffects(t *testing.T) {
	...
	ruleStore := &stubRuleStore{rules: []types.Rule{
		{
			RuleID:     blockID,
			Namespace:  "default",
			Surface:    "home",
	...
	data := &CandidateData{
		Candidates: []types.ScoredItem{{ItemID: "a", Score: 1.0}, {ItemID: "b", Score: 1.0}, {ItemID: "c", Score: 1.0}},
