-- name: popularity_top_k
--
-- description: Rank items by time-decayed event activity, weighted by event
-- type and half-life.
--
-- inputs:
--  1. org_id (uuid)
--  2. namespace (text)
--  3. half_life_days (float8)
--  4. k (int)
--  5. created_after (timestamptz|null)
--  6. min_price (float8|null)
--  7. max_price (float8|null)
--  8. include_tags (text[])
--  9. exclude_ids (text[])
...
WHERE e.org_id = $1
  ...
  AND (
    COALESCE($8::text [], '{}'::text []) = '{}'::text []
    OR i.tags && $8::text []
  ) -- optional tag overlap
  AND (
    COALESCE($9::text [], '{}'::text []) = '{}'::text []
    OR NOT (i.item_id = ANY($9::text []))
  ) -- optional exclusions
GROUP BY e.item_id
ORDER BY score DESC
LIMIT $4;