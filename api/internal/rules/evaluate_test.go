...
func TestEvaluatorPrecedenceBlockPinBoost(t *testing.T) {
	...
	req := EvaluateRequest{
		Candidates: []types.ScoredItem{{ItemID: "a", Score: 1.0}, {ItemID: "b", Score: 1.0}},
...
func TestEvaluatorPinSlotsCap(t *testing.T) {
	...
	req := EvaluateRequest{
		Candidates: []types.ScoredItem{{ItemID: "a", Score: 1}, {ItemID: "b", Score: 1}, {ItemID: "c", Score: 1}},
	}
	res, err := eval.apply(rules, req)
	if err != nil {
		t.Fatalf("apply: %v", err)
...
func TestEvaluatorBoostAdjustsScore(t *testing.T) {
	...
	eval := evaluator{maxPinSlots: 3}
	req := EvaluateRequest{
		Candidates: []types.ScoredItem{{ItemID: "a", Score: 1.0}},
	}
	res, err := eval.apply(rules, req)
	if err != nil {
		t.Fatalf("apply: %v", err)
