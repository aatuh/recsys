...
| `BLEND_SEGMENT_OVERRIDES`                       | Per-segment blend weights (`segment=alpha\|beta\|gamma`). | Legacy bootstrap: prefer segment profiles via `/v1/admin/recommendation/config` / env profile manager (see README tuning playbook). | Starter profiles, `PROFILE_*` knobs, namespace overrides.                                       | Not via request; configure via segment profiles.                           |
