...
import { Section, Row, Button } from "../primitives/UIComponents";
import {
  ConfigService,
...
function SimpleTable({
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      ...
      >
        <thead>
          <tr style={{ backgroundColor: color.panelSubtle }}>
            {columns.map((col) => (
              <th
                key={col.key}
                style={{
...
export function SegmentsSection({
  ...
  return (
    <Section title="Segments Management">
      ...
      <div>
        <h4 style={{ margin: "0 0 16px 0", color: color.text }}>
          ...
          (segment) =>
            ...
            segment.rules.length > 0 && (
              <div key={segment.segment_id} style={{ marginTop: 24 }}>
                <h5 style={{ margin: "0 0 8px 0", color: color.text }}>
