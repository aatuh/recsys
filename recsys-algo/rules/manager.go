...
type Store interface {
	ListActiveRulesForScope(ctx context.Context, orgID uuid.UUID, namespace, surface, segmentID string, ts time.Time) ([]Rule, error)
}
...
func cacheKey(orgID uuid.UUID, namespace, surface, segmentID string) string {
	return orgID.String() + "|" + namespace + "|" + surface + "|" + segmentID
}

// Invalidate removes cached rules for a namespace/surface pair (any segment) for the org.
func (m *Manager) Invalidate(orgID uuid.UUID, namespace, surface string) {
	if m == nil {
		return
	}
	orgKey := orgID.String() + "|"
	ns := strings.TrimSpace(namespace)
	sf := strings.TrimSpace(surface)
	keyPrefix := orgKey
	if ns != "" {
		keyPrefix = orgKey + ns + "|"
		if sf != "" {
			keyPrefix = keyPrefix + sf + "|"
		}
	}
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) loadRules(ctx context.Context, orgID uuid.UUID, namespace, surface, segmentID string, now time.Time) ([]Rule, error) {
	key := cacheKey(orgID, namespace, surface, segmentID)
	m.mu.RLock()
	entry, ok := m.cache[key]
	if ok && now.Sub(entry.fetchedAt) < m.opts.RefreshInterval {
		rulesCopy := make([]Rule, len(entry.rules))
...
func (m *Manager) Evaluate(ctx context.Context, req EvaluateRequest) (*EvaluateResult, error) {
	if m == nil || !m.opts.Enabled {
		return &EvaluateResult{
			Candidates:  append([]recmodel.ScoredItem(nil), req.Candidates...),
			Pinned:      nil,
			Matches:     nil,
	...
	return eval.apply(ctx, rules, req)
}
