...
This directory contains production-ready Docker configuration for the demo-ui application.
...
docker build -f Dockerfile.prod -t recsys-demo-ui:prod .
...
```bash
# Check build logs
docker build -f Dockerfile.prod -t recsys-demo-ui:prod . --no-cache

# Verify build output
docker run --rm recsys-demo-ui:prod ls -la /app/dist
```
