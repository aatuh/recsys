...
## F. Analysis scripts and tooling

### [ ] DOC-301 – Add an “Analysis Scripts Catalog” reference doc

**Background**
There are many utilities under `analysis/scripts/` (seeding, scenarios, tuning, guardrails, load testing). Today they are discoverable only by reading code or scattered mentions in various docs, which is hard for customer stakeholders and their senior developers to navigate.

**Tasks**

1. Create `docs/analysis_scripts_reference.md`.
2. Organize it into sections by intent:
   * **Seeding & reset** – e.g. `seed_dataset.py`, `reset_namespace.py`.
   * **Evaluation & scenarios** – e.g. `run_quality_eval.py`, `run_scenarios.py`, `run_simulation.py`.
   * **Env & profiles** – e.g. `configure_env.py`, `env_profile_manager.py`, `check_env_profiles.py`.
   * **Tuning & guardrails** – e.g. `tuning_harness.py`, `ai_optimizer.py`, `check_guardrails.py`, `profile_coverage.py`.
   * **Reliability & load** – e.g. `check_determinism.py`, Python load tester, `exposure_dashboard.py`, `test_rules.py`.
3. For each script, add a compact table row or bullet with:
   * Script path (for example, `analysis/scripts/run_quality_eval.py`).
   * One-sentence purpose in plain language.
   * Who it is for (e.g. “senior developer / data scientist”, “DevOps/on-call”).
   * When to use it (typical point in the workflow).
   * Key inputs and outputs (e.g. “requires running API and seeded namespace; writes `analysis/quality_metrics.json`”).
   * A single realistic example command.
4. Cross-link from:
   * `docs/tuning_playbook.md` (for tuning-related scripts).
   * `docs/simulations_and_guardrails.md` (for simulation/guardrail scripts).
   * `docs/env_reference.md` (for env/config tooling).
5. Keep the doc written so that a customer business representative can skim the headings to understand *what* exists and a senior developer can immediately see *how* to run each script.

**Definition of Done**

* `docs/analysis_scripts_reference.md` exists and lists all major `analysis/scripts/*.py` tools plus their roles.
* Tuning, simulations/guardrails, and env docs link to this reference instead of re-describing each script in isolation.
* A senior developer can discover the right script for a task by scanning this single document.

---

### [ ] DOC-302 – Add flow-based “journeys” that reference analysis scripts

**Background**
While individual scripts are powerful, what customers and their senior developers need is guided “journeys” (e.g. “first evaluation”, “tune a tenant”, “investigate regression”) that say which scripts to run, in what order, and why.

**Tasks**

1. Update or extend existing flow docs to explicitly tie scripts to steps:
   * **Onboarding / first evaluation** – in `GETTING_STARTED.md` and/or `docs/overview.md`, add a short section “Run an evaluation locally” that maps steps to scripts:
     * Reset → `reset_namespace.py`.
     * Seed → `seed_dataset.py`.
     * Quality eval → `run_quality_eval.py`.
     * Scenarios → `run_scenarios.py`.
   * **Tuning & guardrails** – in `docs/tuning_playbook.md`, ensure each phase calls out the relevant scripts:
     * Capture/apply profiles → `env_profile_manager.py`.
     * Run sweeps → `tuning_harness.py`.
     * Suggest next parameters → `ai_optimizer.py`.
     * Enforce guardrails on tuning runs → `check_guardrails.py`.
   * **Ops / reliability** – in `docs/simulations_and_guardrails.md` and/or `docs/rules-runbook.md`, add a short “Reliability checks” section:
     * Determinism check → `check_determinism.py`.
     * Load test → Python load tester in `analysis/scripts/recommendations_load.py`.
     * Exposure visualization → `exposure_dashboard.py`.
2. For each journey, write a brief narrative:
   * What problem the journey solves (e.g. “Validate a new tenant before go-live”).
   * Which scripts are involved, with links to `docs/analysis_scripts_reference.md` for details.
3. Ensure customer-facing docs avoid jargon where possible, or link to `docs/concepts_and_metrics.md` when metrics/terms are mentioned.

**Definition of Done**

* Existing flow docs (GETTING_STARTED, tuning playbook, simulations/guardrails, rules runbook) explicitly mention which `analysis/scripts/*.py` tools they expect the reader to use.
* Each journey includes at least a short, business-friendly “why” and a developer-friendly “what to run”.
* There is no need to guess which script to use at any stage of a typical integration or tuning process.
